...
//work for both shopify and db data
CompareData.prototype.sortVariants = function(variants,key) {
	...
	let promise = new Promise((resolve,reject) => {
		...
		if(index < shopifyProducts.length){
			...
			.then(product => {
				...
				console.log("product from db: ",product[0].product_title);
				console.log('product from shopify: ',shopifyProducts[index].title);
				let sortedShopifyProduct = shopifyProducts[index];
				//sortedShopifyProduct.variants = this.sortVariants(sortedShopifyProduct.variants,'sku');
				let sortedProduct = product[0];
				//sortedProduct.variant_data = this.sortVariants(sortedProduct.variant_data,'item_code');
				let compareError = this.compareProducts(sortedShopifyProduct,sortedProduct);
